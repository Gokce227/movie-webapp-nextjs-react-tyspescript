...
import { getMovieDetailById } from '@/app/redux/slices/MovieDetailSlice';
import Image from 'next/image';
import { FaStar } from 'react-icons/fa';
import { API_IMG, MovieFavorite } from '@/app/constants';
import Loading from '@/app/loading';
import Error from '@/app/error';
import { addToSaveMovie,removeFromSaveMovie, addToFavoriteMovie, removeFromFavoriteMovie } from '@/app/redux/slices/FavoriteSlice';
import { Heart, HeartOff } from 'lucide-react'; 
import { FaRegBookmark } from "react-icons/fa6";

const MovieDetailPage = () => {
    const dispatch = useDispatch<AppDispatch>();
    const { movieDetail, status, error } = useSelector((state: RootState) => state.movieDetail);
    const { favoriteMovies, saveMovies } = useSelector((state: RootState) => state.favorite);
...
//---------------------------------------------------------Favorite-------------------------
    const isFavorite = favoriteMovies?.some((movie) => movie.id === Number(id));

    const handleAddFavorite = () => {
        if (!title || !poster_path || vote_average === undefined) return;

        const payload: MovieFavorite = {
            id: Number(id),
            title,
            poster_path,
            vote_average,
        ...
        if (!favoriteMovies.some((movie) => movie.id === Number(id))) {
            dispatch(addToFavoriteMovie(payload));
        }
    };

    const handleRemoveFavorite = () => {
        dispatch(removeFromFavoriteMovie({ id: Number(id) }));
    };

//----------------------------------------------------Saved---------------------------------
    
    const isSaved = saveMovies?.some((movie) => movie.id === Number(id));
    ...
    const handleAddSave = () => {
        ...
        if (!saveMovies.some((movie) => movie.id === Number(id))) {
            dispatch(addToSaveMovie(payload)); // optional: favoriyle aynıysa kaldırabilirsin
        }
    };

    const handleRemoveSave = () => {
        dispatch(removeFromSaveMovie({ id: Number(id) }));
    };
        ...
        <div className="relative min-h-screen py-10 px-6 overflow-hidden text-gray-900">
            ...
            <div className="relative z-10 max-w-5xl mx-auto rounded-xl p-6 md:flex gap-8 transition-all duration-300 group hover:scale-[1.015] hover:shadow-2xl hover:backdrop-blur-lg bg-white/60 shadow-lg backdrop-blur-md">
                ...
                <div className="mt-4 md:mt-0 flex-1">
                    <h1 className="text-3xl font-bold bg-gradient-to-r from-[#0f0c29] via-[#302b63] to-[#24243e] bg-clip-text text-transparent mb-3">{title}</h1>

                    <p className="text-sm text-gray-800 mb-2">🎬 Publication date {release_year.getFullYear()}</p>
                    ...
                    {/*  Add/Remove Favorite Button */}
                        <div className="mt-6 flex justify-center gap-3">
                            ...
                            <button
                                ...
                                className="group inline-flex items-center gap-2 px-5 py-2 rounded-lg border border-orange-400 text-orange-500 hover:bg-orange-50 transition duration-200"
                            >
                            ...
                            <button
                                ...
                                className="group inline-flex items-center gap-2 px-5 py-2 rounded-lg border border-blue-400 text-blue-600 hover:bg-blue-50 transition duration-200"
                            >
                              <FaRegBookmark className="w-5 h-5 group-hover:scale-110 transition-transform duration-200" />
                                Save for Later
                            </button>
